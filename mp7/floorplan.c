...
/* intro paragraph: in this week's mp, we are supposed to wirte several functions that constitute a 
  program called Floorplan. Floorplanning is important in physical design, and this program will evaluate 
  the rectangular area and optimizing the total areas.
  the most important function in this program is slicing_tree, which will create a left-skewed tree 
  from a horizontal placed floorplan. 
  other functions including: is_leaf_node, is_internal_node, is_in_subtree, rotate, recut, 
  swap_module, swap_topoogy, get_expression, init_slicing_tree, get_total_resources. 
*/
...
void rotate(node_t* ptr) {
  // swap the height and the weight
...
void recut(node_t* ptr) {
  ...
  // if V change to H, and if H change to V
  if (ptr->cutline == V){
    ptr->cutline = H;
  } else {
    ptr->cutline = V;
...
void swap_module(node_t* a, node_t* b) {
  ...
  // swap two modules of two leave nodes
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // check if the node is on right or left
  int aisatleft;
  int bisatleft;
  if (a->parent->left == a) {
    aisatleft = 1;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  if (ptr->cutline == UNDEFINED_CUTLINE) {
    (expression+count)->module = ptr->module;
    (expression+count)->cutline = UNDEFINED_CUTLINE;
    count++;
  } else {
    (expression+count)->module = NULL;
    (expression+count)->cutline = ptr->cutline;
    count++;
  }
  *nth = count;
}
...
{ 
  ...
  // recursive case
  int count = get_total_resource(ptr->left) + get_total_resource(ptr->right) + 1;
  if (ptr->module != NULL) {
    sum = sum + ptr->module->resource;
  }
  if (count > 1){
    return sum;
  }
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // allocate memory space for root
  node_t *root = (node_t *)malloc(sizeof(node_t));
  // if not the last level, set root to V
  if (n < num_modules - 1) {
    root->cutline = V;
    root->module = NULL;
    root->parent = par;
    ...
    root->module = &modules[num_modules - 1];
    root->left = NULL;
    root->right = NULL;
    return root;
  }
